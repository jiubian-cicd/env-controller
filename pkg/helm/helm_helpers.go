...
func AppendMyValues(valueFiles []string) ([]string, error) {
	...
	if exists {
		...
		fmt.Sprintf("Using local value overrides file %s", util.ColorInfo(myValuesFile))
	} else {
...
func InspectChart(chart string, version string, repo string, username string, password string,
	...
	defer func() {
		...
		if err1 != nil {
			fmt.Sprintf("Error removing %s %v", dir, err1)
		}
	}()
	if err != nil {
...
//		fmt.Sprintf("Updating Helm repository...")
//		err := helmer.UpdateRepo()
//		if err != nil {
//			return errors.Wrap(err, "failed to update repository")
//		}
//		fmt.Sprintf("Helm repository update done.")
//	}
...
//				if err != nil {
//					fmt.Sprintf("Deleting temp file %s", f)
//				}
//			}
//		}
...
//			if err != nil {
//				repoName = uuid.New()
//				fmt.Sprintf("Unable to parse %s as URL so assigning random name %s", helmURL, repoName)
//			} else {
...
//			fmt.Sprintf("Storing credentials for %s in vault %s", repo, RepoVaultPath)
//			creds[repo] = cred
...
//			}
//		} else {
//			fmt.Sprintf("Read credentials for %s from vault %s", repo, RepoVaultPath)
//		}
...
func PromptForRepoCredsIfNeeded(repo string, cred *HelmRepoCredential, in terminal.FileReader,
	...
	for i := 0; true; i++ {
		...
		if i == 4 {
			...
			if cred.Username != "" || cred.Password != "" {
				fmt.Sprintf("Authentication for %s failed (%s/%s)", repo, cred.Username, strings.Repeat("*",
					len(cred.Password)))
			}
...
func FindLatestChart(name string, helmer Helmer) (*ChartSummary, error) {
	...
	fmt.Sprintf("found %d versions: %#v", len(info), info)
	return &info[0], nil
}
