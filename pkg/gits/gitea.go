...
func (p *GiteaProvider) ForkRepository(originalOrg string, name string, destinationOrg string) (*GitRepository, error) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "try again later") {
			fmt.Sprintf("Waiting for the fork of %s/%s to appear...", owner, name)
...
func (p *GiteaProvider) CreateWebHook(data *GitWebHookArguments) error {
	...
	fmt.Sprintf("Creating Gitea webhook for %s/%s for url %s", util.ColorInfo(owner), util.ColorInfo(repo), util.ColorInfo(webhookUrl))
...
func (p *GiteaProvider) AddCollaborator(user string, organisation string, repo string) error {
	fmt.Sprintf("Automatically adding the pipeline user as a collaborator is currently not implemented for Gitea. Please add user: %v as a collaborator to this project.", user)
	return nil
}

func (p *GiteaProvider) ListInvitations() ([]*github.RepositoryInvitation, *github.Response, error) {
	fmt.Sprintf("Automatically adding the pipeline user as a collaborator is currently not implemented for Gitea.")
