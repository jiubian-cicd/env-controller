...
func (g *GitCLI) clone(dir string, gitURL string, remoteName string, shallow bool, verbose bool, localBranch string,
	...
	var err error
	if verbose {
		fmt.Sprintf("cloning repository %s to dir %s", gitURL, dir)
	}
	if remoteName == "" {
		remoteName = "origin"
	}
	...
	if verbose {
		fmt.Sprintf("ran git init in %s", dir)
	}
	...
	if verbose {
		fmt.Sprintf("ran git add remote %s %s in %s", remoteName, gitURL, dir)
	}
	...
	if localBranch != "master" {
		...
		if verbose {
			fmt.Sprintf("ran git branch %s in directory %s", localBranch, dir)
		}
	}
	if commitish == "" {
	...
	if verbose {
		fmt.Sprintf("ran git reset --hard %s in directory %s", commitish, dir)
	}
	...
	if verbose {
		fmt.Sprintf("ran git branch --set-upstream-to %s/%s %s in directory %s", remoteName, commitish,
			localBranch, dir)
	}
	return nil
}
...
func (g *GitCLI) fetchBranch(dir string, repo string, unshallow bool, shallow bool,
	...
	if verbose {
		if shallow {
			fmt.Sprintf("ran git fetch %s --depth=1 %s in dir %s", repo, strings.Join(refspecs, " "), dir)
		} else if unshallow {
			fmt.Sprintf("ran git fetch %s unshallow %s in dir %s", repo, strings.Join(refspecs, " "), dir)
		} else {
			fmt.Sprintf("ran git fetch %s --depth=1 %s in dir %s", repo, strings.Join(refspecs, " "), dir)
		}
