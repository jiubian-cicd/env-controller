...
func (p *GitHubProvider) ListRepositories(org string) ([]*GitRepository, error) {
	...
	if IsOwnerGitHubUser(owner, p.Username) {
		fmt.Sprintf("Owner of repo is same as username, using GitHub API for Users")
...
func (p *GitHubProvider) ForkRepository(originalOrg string, name string, destinationOrg string) (*GitRepository, error) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "try again later") {
			fmt.Sprintf("Waiting for the fork of %s/%s to appear...", destinationOrg, name)
...
func (p *GitHubProvider) CreateWebHook(data *GitWebHookArguments) error {
	...
	if err != nil {
		fmt.Sprintf("Querying webhooks on %s/%s: %s", owner, repo, err)
	}
	for _, hook := range hooks {
		...
		if ok && u == webhookUrl {
			...
			if ok && s != data.Secret {
				...
				fmt.Sprintf("Already has a webhook registered for %s", webhookUrl)
				return nil
			}
		}
	}
	...
	fmt.Sprintf("Creating GitHub webhook for %s/%s for url %s", util.ColorInfo(owner), util.ColorInfo(repo), util.ColorInfo(webhookUrl))
...
func (p *GitHubProvider) UpdateWebHook(data *GitWebHookArguments) error {
	...
	if dataId == 0 {
		for _, hook := range hooks {
			...
			if ok && s == data.ExistingURL {
				fmt.Sprintf("Found existing webhook for url %s", data.ExistingURL)
				dataId = hook.GetID()
			}
		}
	}
	...
	if dataId != 0 {
		...
		fmt.Sprintf("Updating GitHub webhook for %s/%s for url %s", util.ColorInfo(owner), util.ColorInfo(repo), util.ColorInfo(webhookUrl))
		_, _, err = p.Client.Repositories.EditHook(p.Context, owner, repo, dataId, hook)
	} else {
		fmt.Sprintf("No webhooks found to update")
	}
	return err
}
...
func (p *GitHubProvider) asGitHubCommit(commit *github.RepositoryCommit) GitCommit {
	...
	if commit.Commit != nil {
		...
		fmt.Sprintf("No Commit object for for commit: %s", commit.GetSHA())
	}
	var author *GitUser
	if commit.Author != nil {
		...
		fmt.Sprintf("No author for commit: %s", commit.GetSHA())
	}
	return GitCommit{
		Message: message,
		URL:     commit.GetURL(),
	...
	for _, commit := range commits {
		...
		if commit.Author != nil {
			if summary.Author.Email == "" {
				fmt.Sprintf("Commit author email is empty for: %s", commit.GetSHA())
				dir, err := os.Getwd()
				if err != nil {
					return answer, err
				}
				...
				fmt.Sprintf("Looking for commits in: %s", gitDir)
				email, err := p.Git.GetAuthorEmailForCommit(gitDir, commit.GetSHA())
				if err != nil {
					fmt.Sprintf("Commit not found: %s", commit.GetSHA())
					continue
				}
...
func (p *GitHubProvider) UpdateRelease(owner string, repo string, tag string, releaseInfo *GitRelease) error {
	...
	if r != nil && r.StatusCode == 404 {
		fmt.Sprintf("No release found for %s/%s and tag %s so creating a new release", owner, repo, tag)
...
func (p *GitHubProvider) UpdateReleaseStatus(owner string, repo string, tag string, releaseInfo *GitRelease) error {
	...
	if r != nil && r.StatusCode == 404 {
		fmt.Sprintf("No release found for %s/%s and tag %s", owner, repo, tag)
		return err
	}
...
func (p *GitHubProvider) UserInfo(username string) *GitUser {
	...
	if user == nil || err != nil {
		fmt.Sprintf("Unable to fetch user info for %s", username)
		return nil
	}
...
func (p *GitHubProvider) AddCollaborator(user string, organisation string, repo string) error {
	fmt.Sprintf("Automatically adding the pipeline user: %v as a collaborator.", user)
...
func (p *GitHubProvider) AcceptInvitation(ID int64) (*github.Response, error) {
	fmt.Sprintf("Automatically accepted invitation: %v for the pipeline user.", ID)
...
func (p *GitHubProvider) IsWikiEnabled(owner string, repo string) (bool, error) {
	...
	defer func() {
		...
		if err != nil {
			fmt.Sprintf("unable to delete temp dir %s", dir)
		}
	}()
	if err != nil {
