...
func Unshallow(dir string, gitter Gitter) error {
	...
	if shallow {
		...
		fmt.Sprintf("Converted %s to an unshallow repository", dir)
		return nil
	}
	return nil
}
...
func FetchAndMergeSHAs(SHAs []string, baseBranch string, baseSha string, remote string, dir string,
	...
	if err != nil {
		...
		if err != nil {
			...
			fmt.Sprintf("ran %s in %s", util.ColorInfo("git remote update"), dir)
		}
		fmt.Sprintf("ran git fetch %s %s in %s", remote, strings.Join(refspecs, " "), dir)

		err = Unshallow(dir, gitter)
		if err != nil {
			return errors.WithStack(err)
		}
		fmt.Sprintf("Unshallowed git repo in %s", dir)
	} else {
		fmt.Sprintf("ran git fetch --unshallow %s %s in %s", remote, strings.Join(refspecs, " "), dir)
	}
	...
	fmt.Sprintf("ran git checkout %s in %s", baseBranch, dir)
	...
	fmt.Sprintf("ran git reset --hard %s in %s", baseSha, dir)
	err = gitter.CleanForce(dir, ".")
	if err != nil {
		return errors.Wrapf(err, "cleaning up the git repo")
	}
	fmt.Sprintf("ran clean --force -d . in %s", dir)
	// Now do the merges
	for _, sha := range SHAs {
		...
		fmt.Sprintf("ran git merge %s in %s", sha, dir)
...
func GitProviderURL(text string) string {
	...
	if err != nil {
		fmt.Sprintf("failed to parse git provider URL %s: %s", text, err.Error())
		return text
	}
	u.Path = ""
...
func PushRepoAndCreatePullRequest(dir string, upstreamRepo *GitRepository, forkRepo *GitRepository, base string, prDetails *PullRequestDetails, filter *PullRequestFilter, commit bool, commitMessage string, push bool, dryRun bool, gitter Gitter, provider GitProvider, labels []string) (*PullRequestInfo, error) {
	...
	if commit {
		...
		if !changed {
			fmt.Sprintf("No changes made to the source code in %s. Code must be up to date!", dir)
			return nil, nil
		}
		if commitMessage == "" {
	...
	if filter != nil && push {
		...
		if len(existingPrs) > 1 {
			...
			fmt.Sprintf("Found more than one PR %s using filter %s on repo %s/%s so rebasing latest PR %s", strings.Join(prs, ", "), filter.String(), upstreamRepo.Organisation, upstreamRepo.Name, existingPrs[:1][0].URL)
	...
	if dryRun {
		fmt.Sprintf("Commit created but not pushed; would have updated pull request %s with %s and used commit message %s. Please manually delete %s when you are done", util.ColorInfo(existingPr.URL), prDetails.String(), commitMessage, util.ColorInfo(dir))
		return nil, nil
	} else if push {
	...
	if existingPr != nil {
		...
		fmt.Sprintf("Updated Pull Request: %s", util.ColorInfo(pr.URL))
	} else {
		...
		fmt.Sprintf("Created Pull Request: %s", util.ColorInfo(pr.URL))
	}
	if labels != nil {
		number := *pr.Number
		var err error
		...
		fmt.Sprintf("Added label %s to Pull Request %s", util.ColorInfo(strings.Join(labels, ", ")), pr.URL)
	}
...
func ForkAndPullRepo(gitURL string, dir string, baseRef string, branchName string, provider GitProvider, gitter Gitter, forkName string) (string, string, *GitRepository, *GitRepository, error) {
	...
	if fork {
		...
		if err != nil {
			fmt.Sprintf(errors.Wrapf(err, "getting repository %s/%s", username, forkName).Error())
			...
			fmt.Sprintf("Forked Git repository to %s\n", util.ColorInfo(forkInfo.HTMLURL))
		}
	...
	if len(toCherryPick) > 0 {
		fmt.Sprintf("Attempting to cherry pick commits that were on %s but not yet pushed", localBranchName)
	}
	for _, c := range toCherryPick {
		err = gitter.CherryPick(dir, c.SHA)
		if err != nil {
			if IsEmptyCommitError(err) {
				fmt.Sprintf("  Ignoring %s as is empty", c.OneLine())
				err = gitter.Reset(dir, "", true)
				if err != nil {
					return "", "", nil, nil, errors.Wrapf(err, "running git reset --hard")
				}
			} else {
				fmt.Sprintf(errors.Wrapf(err, "Unable to cherry-pick %s", util.ColorWarning(c.SHA)).Error())
			}
		} else {
			fmt.Sprintf("  Cherry-picked %s", c.OneLine())
		}
	}
	if len(toCherryPick) > 0 {
		fmt.Sprintf("")
...
func DuplicateGitRepoFromCommitsh(toOrg string, toName string, fromGitURL string, fromCommitish string, toBranch string, private bool, provider GitProvider, gitter Gitter) (*GitRepository, error) {
	...
	if err != nil {
		fmt.Sprintf(errors.Wrapf(err, "getting repository %s/%s", toOrg, toName).Error())
