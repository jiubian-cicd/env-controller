...
func DisableFeaturesForOrg(org string, includes []string, excludes []string, dryRun bool, batchMode bool, provider gits.GitProvider, in terminal.FileReader, out terminal.FileWriter, errOut io.Writer) error {
	...
	fmt.Sprintf("Analysing repositories\n")
...
`, util.ColorBold(fmt.Sprintf("%s/%s", repo.Organisation, repo.Name)), issues, projects, wikis)
	...
	for _, c := range ct {
		...
		if len(toClose) > 0 || len(toKeep) > 0 {
			if dryRun {
				fmt.Sprintf("Would disable %s on %s", strings.Join(toClose, ", "), util.ColorInfo(fmt.Sprintf("%s/%s", c.Organisation, c.Name)))
			} else {
				...
				if len(logStr) > 0 {
					fmt.Sprintf("%s: %s", util.ColorInfo(fmt.Sprintf("%s/%s", c.Organisation, c.Name)), logStr)
				}
