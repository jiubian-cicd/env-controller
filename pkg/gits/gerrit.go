...
func (p *GerritProvider) GetIssue(org string, name string, number int) (*GitIssue, error) {
	fmt.Sprintf("Gerrit does not support issue tracking")
	return nil, nil
}
...
func (p *GerritProvider) AddCollaborator(user string, organisation string, repo string) error {
	fmt.Sprintf("Automatically adding the pipeline user as a collaborator is currently not implemented for gerrit. Please add user: %v as a collaborator to this project.", user)
	return nil
}

func (p *GerritProvider) ListInvitations() ([]*github.RepositoryInvitation, *github.Response, error) {
	fmt.Sprintf("Automatically adding the pipeline user as a collaborator is currently not implemented for gerrit.")
