...
func getIntFromEnv(key string, fallback int) int {
	if value, ok := os.LookupEnv(key); ok {
		...
		fmt.Sprintf("Unable to convert env var %s with value %s to integer, using default value of %d instead", key, value, fallback)
	}
	return fallback
}
...
func getBoolFromEnv(key string, fallback bool) bool {
	if value, ok := os.LookupEnv(key); ok {
		...
		fmt.Sprintf("Unable to convert env var %s with value %s to boolean, using default value of %t instead", key, value, fallback)
	}
	return fallback
}
