...
func GetLatestReleaseFromGitHub(githubOwner, githubRepo string) (string, error) {
	...
	if version == "" || err != nil {
		fmt.Sprintf("getting latest release using HTTP redirect (%v) - using API instead", err)
...
func GetLatestReleaseFromGitHubURL(gitURL string) (string, error) {
	...
	if !strings.HasPrefix(gitURL, gitHubPrefix) {
		fmt.Sprintf("cannot determine the latest release of version stream git URL %s\n", gitURL)
		return "", nil
	}
	name := strings.TrimPrefix(gitURL, gitHubPrefix)
	paths := strings.Split(name, "/")
	if len(paths) <= 1 {
		fmt.Sprintf("cannot parse git URL %s so cannot determine the latest release\n", gitURL)
		return "", nil
	}
	owner := paths[0]
...
func getLatestReleaseFromHostUsingHttpRedirect(host, githubOwner, githubRepo string) (string, error) {
	...
	// https://github.com/jiubian-cicd/env-controller/releases/tag/v1.3.696
