...
func (o *CommonOptions) EnsureServiceAccount(ns string, serviceAccountName string) error {
	...
	if err != nil {
		...
		fmt.Sprintf("Created ServiceAccount %s in namespace %s", util.ColorInfo(serviceAccountName), util.ColorInfo(ns))
	}
	return err
}
...
func (o *CommonOptions) EnsureClusterRoleExists(roleName string, namespace string) error {
	fmt.Sprintf("Ensuring cluster role exists, role name: %s, namespace: %s", roleName, namespace)
	...
	if err != nil {
		fmt.Sprintf("Trying to create ClusterRole %s in namespace %s", roleName, namespace)
		...
		fmt.Sprintf("Created ClusterRole %s in namespace %s", roleName, namespace)
	}
	return nil
}
...
func (o *CommonOptions) EnsureClusterRoleBinding(clusterRoleBindingName string, role string, serviceAccountNamespace string, serviceAccountName string) error {
	...
	if err != nil {
		fmt.Sprintf("Trying to create ClusterRoleBinding %s for role: %s and ServiceAccount: %s/%s",
		...
		fmt.Sprintf("Created ClusterRoleBinding %s", clusterRoleBindingName)
	}
	return nil
}
