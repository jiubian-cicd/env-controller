...
import (
	...
	"github.com/jiubian-cicd/env-controller/pkg/secreturl/localvault"
	"io/ioutil"
	"net/url"
	"os"
	"path/filepath"
...
func (o *CommonOptions) InitHelm(config InitHelmConfig) error {
	...
	if config.Helm3 {
		fmt.Sprintf("Using %s", util.ColorInfo("helm3"))
		skipTiller = true
	} else {
		fmt.Sprintf("Using %s", util.ColorInfo("helm2"))
	}
	if !skipTiller {
		fmt.Sprintf("Configuring %s", util.ColorInfo("tiller"))
		...
		if config.GlobalTiller {
			...
			if err != nil {
				...
				fmt.Sprintf("Created Role %s in namespace %s", util.ColorInfo(roleName), util.ColorInfo(tillerNamespace))
			}
			...
			if err != nil {
				...
				fmt.Sprintf("Created RoleBinding %s in namespace %s", util.ColorInfo(roleName), util.ColorInfo(tillerNamespace))
			}
		}

		running, err := kube.IsDeploymentRunning(client, "tiller-deploy", tillerNamespace)
		if running {
			fmt.Sprintf("Tiller Deployment is running in namespace %s", util.ColorInfo(tillerNamespace))
			return nil
		}
		if err == nil && !running {
		...
		if !running {
			fmt.Sprintf("Initialising helm using ServiceAccount %s in namespace %s", util.ColorInfo(serviceAccountName), util.ColorInfo(tillerNamespace))
		...
		fmt.Sprintf("Waiting for tiller-deploy to be ready in tiller namespace %s", tillerNamespace)
		err = kube.WaitForDeploymentToBeReady(client, "tiller-deploy", tillerNamespace, 10*time.Minute)
		if err != nil {
			return err
		}
	} else {
		fmt.Sprintf("Skipping %s", util.ColorInfo("tiller"))
	}
...
func (o *CommonOptions) AddHelmBinaryRepoIfMissing(url, repoName, username, password string) (string, error) {
	name, err := helm.AddHelmRepoIfMissing(url, repoName, username, password, o.Helm(), nil, o.In, o.Out, o.Err)
	if err != nil {
...
//			if err != nil {
//				fmt.Sprintf("deleting file %s: %v", fileName.Name(), err)
//			}
//		}()
//		if err != nil {
...
//	if err != nil {
//		fmt.Sprintf("unable to fetch chart - %s", err)
//	}
//
...
func (o *CommonOptions) InstallChartWithOptions(options helm.InstallChartOptions) error {
	return o.InstallChartWithOptionsAndTimeout(options, "6000")
}
...
func (o *CommonOptions) InstallChartWithOptionsAndTimeout(options helm.InstallChartOptions, timeout string) error {
	...
	if options.VersionsDir == "" {
		dir, err := os.Getwd()
		if err != nil {
			log.Logger().Errorf("getwd error : %s", err.Error())
		}
		options.VersionsDir = filepath.Join(dir, "envctl-versions")
	}
...
//	}
//
//	fmt.Sprintf("Current configuration dir: %s", configDir)
//	fmt.Sprintf("versionRepository: %s git ref: %s", versionRepository, versionRef)
//
...
//		if err != nil {
//			fmt.Sprintf("Error opening %s", wrkDir)
//			return o.deleteAndReClone(wrkDir, versionRepository, versionRef, o.Out)
//		}
//		remote, err := repo.Remote("origin")
//		if err != nil {
//			fmt.Sprintf("Error getting remote origin")
//			return o.deleteAndReClone(wrkDir, versionRepository, versionRef, o.Out)
//		}
...
//			if err != nil {
//				fmt.Sprintf("Error confirming if we should pull latest, skipping %s", wrkDir)
//			}
//		} else {
//			pullLatest = true
//			fmt.Sprintf(util.QuestionAnswer("A local Jenkins X versions repository already exists, pulling the latest", util.YesNo(pullLatest)))
//		}
//		if pullLatest {
//			w, err := repo.Worktree()
//			if err == nil {
...
func (o *CommonOptions) deleteAndReClone(wrkDir string, versionRepository string, referenceName string, fw terminal.FileWriter) (string, error) {
	fmt.Sprintf("Deleting and cloning the Jenkins X versions repo")
...
func (o *CommonOptions) clone(wrkDir string, versionRepository string, referenceName string, fw terminal.FileWriter) error {
	if referenceName == "" || referenceName == "master" {
		...
		if strings.HasPrefix(referenceName, "PR-") {
			...
			fmt.Sprintf("Cloning the Jenkins X versions repo %s with PR: %s to %s", util.ColorInfo(versionRepository), util.ColorInfo(referenceName), util.ColorInfo(wrkDir))
			return o.shallowCloneGitRepositoryToDir(wrkDir, versionRepository, prNumber, "")
		}
		fmt.Sprintf("Cloning the Jenkins X versions repo %s with revision %s to %s", util.ColorInfo(versionRepository), util.ColorInfo(referenceName), util.ColorInfo(wrkDir))
	...
	fmt.Sprintf("Cloning the Jenkins X versions repo %s with ref %s to %s", util.ColorInfo(versionRepository), util.ColorInfo(referenceName), util.ColorInfo(wrkDir))
...
func (o *CommonOptions) shallowCloneGitRepositoryToDir(dir string, gitURL string, pullRequestNumber string, revision string) error {
	if pullRequestNumber != "" {
		fmt.Sprintf("shallow cloning pull request %s of repository %s to temp dir %s", gitURL,
		...
		fmt.Sprintf("shallow cloning revision %s of repository %s to temp dir %s", gitURL,
			revision, dir)
		...
		fmt.Sprintf("shallow cloning master of repository %s to temp dir %s", gitURL, dir)
...
func (o *CommonOptions) ModifyHelmValuesFile(dir string, fn func(string) (string, error)) error {
	...
	fmt.Sprintf("modified the helm file: %s", util.ColorInfo(valuesFileName))
	return nil
}
