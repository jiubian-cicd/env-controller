...
func (v *VersionResolver) ResolveGitVersion(gitURL string) (string, error) {
	...
	if answer == "" {
		...
		fmt.Sprintf("could not find a stable version for git repository: %s in %s", gitURL, v.VersionsDir)
		fmt.Sprintf("for background see: https://jenkins-x.io/architecture/version-stream/")
		fmt.Sprintf("please lock this version down via the command: %s", util.ColorInfo(fmt.Sprintf("jx step create version pr -k git -n %s -v 1.2.3", path)))
	}
	return answer, nil
}
