...
func FindServiceURL(client kubernetes.Interface, namespace string, name string) (string, error) {
	fmt.Sprintf("finding service url for %s in namespace %s", name, namespace)
	...
	if answer != "" {
		fmt.Sprintf("found service url %s", answer)
		return answer, nil
	}

	fmt.Sprintf("couldn't find service url, attempting to look up via ingress")

	// lets try find the service via Ingress
	ing, err := client.ExtensionsV1beta1().Ingresses(namespace).Get(name, meta_v1.GetOptions{})
	if err != nil {
		fmt.Sprintf("error finding ingress for %s in namespace %s - err %s", name, namespace, err)
		return "", nil
	}

	url := IngressURL(ing)
	if url == "" {
		fmt.Sprintf("unable to find service url via ingress for %s in namespace %s", name, namespace)
	}
	return url, nil
}
		...
		if len(ing.Spec.Rules) > 0 {
			...
			for _, tls := range ing.Spec.TLS {
				for _, h := range tls.Hosts {
					if h != "" {
						url := "https://" + h
						fmt.Sprintf("found service url %s", url)
						return url
					}
				}
			}
