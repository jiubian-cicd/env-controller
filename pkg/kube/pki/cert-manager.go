...
func alreadyConfigured(certClient certclient.Interface, targetNamespace string, ingressConfig kube.IngressConfig) bool {
	...
	if err != nil {
		fmt.Sprintf("Certificate issuer %s does not exist. Creating...", util.ColorInfo(ingressConfig.Issuer))
		return false
	}
	...
	if issuer.Spec.ACME.Email != ingressConfig.Email {
		...
		if err != nil {
			...
			fmt.Sprintf("Certificate issuer %s can not be updated. Recreating...", util.ColorInfo(ingressConfig.Issuer))
			return false
		}
	}
	fmt.Sprintf("Certificate issuer %s already configured.", util.ColorInfo(ingressConfig.Issuer))
	return true
}
